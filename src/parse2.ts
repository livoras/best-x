 ...
 * @returns 处理后的HTML字符串
 */
export async function parseHtml(htmlContent: string): Promise<string> {
  // 使用 cheerio 解析 HTML，和 parse.ts 保持一致
  const cheerio = await import('cheerio');
  const $ = cheerio.load(htmlContent);
  
  // 1. 生成XPath并添加xp属性（在清理之前进行，基于原始DOM）
  // 遍历所有元素，为有价值的元素生成XPath（使用utils中的函数）
  $('*').each(function() {
    const element = $(this);
...
export async function parseHtmlCli(inputFile?: string, outputFile?: string): Promise<void> {
  ...
  const reduction = Math.round((1 - result.length / htmlContent.length) * 100);
